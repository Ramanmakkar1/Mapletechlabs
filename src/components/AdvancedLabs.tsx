        ...
        <div className="advanced-labs-grid">
          <div className="reveal">
            ...
            <button style={{ marginTop: 52, display: 'inline-flex', alignItems: 'center', gap: 10, height: 52, padding: '0 28px', borderRadius: 100, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 500, cursor: 'pointer', transition: '0.3s', fontFamily: 'inherit' }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(180,253,131,0.1)'; e.currentTarget.style.borderColor = 'rgba(180,253,131,0.3)'; e.currentTarget.style.color = '#b4fd83'; }}
          ...
          <div className="reveal reveal-d1" style={{ position: 'relative' }}>
            <div style={{ borderRadius: 48, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', background: '#050505', aspectRatio: '4/5', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ position: 'absolute', top: '20%', left: '20%', width: '60%', height: '60%', background: 'radial-gradient(circle, rgba(180,253,131,0.15) 0%, transparent 65%)', filter: 'blur(60px)' }} />
              <div style={{ position: 'relative', zIndex: 2, width: '80%', aspectRatio: '1' }}>
                <svg viewBox="0 0 400 400" style={{ width: '100%', opacity: 0.6 }}>
                  <circle cx="200" cy="200" r="160" fill="none" stroke="rgba(180,253,131,0.15)" strokeWidth="1" strokeDasharray="4 8" />
                  <circle cx="200" cy="200" r="110" fill="none" stroke="rgba(180,253,131,0.08)" strokeWidth="1" strokeDasharray="3 6" />
                  {([[200,40],[340,120],[340,280],[200,360],[60,280],[60,120]] as [number,number][]).map(([cx, cy], i) => (
                    <g key={i}>
                      <circle cx={cx} cy={cy} r="8" fill="rgba(180,253,131,0.3)" />
                      <circle cx={cx} cy={cy} r="16" fill="none" stroke="rgba(180,253,131,0.1)" strokeWidth="1" />
                      <line x1={cx} y1={cy} x2="200" y2="200" stroke="rgba(180,253,131,0.06)" strokeWidth="1" />
                    </g>
                  ))}
                  <circle cx="200" cy="200" r="30" fill="rgba(180,253,131,0.1)" />
                  <circle cx="200" cy="200" r="14" fill="#b4fd83" opacity="0.4" />
                  <circle cx="200" cy="200" r="6" fill="#b4fd83" />
                </svg>
              </div>
              <div style={{ position: 'absolute', bottom: 28, left: 28, display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 100, padding: '9px 18px' }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: '#b4fd83', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Neural Core Active</span>
              </div>
            </div>
          </div>
        </div>
